...
export function ExpenseFormPage() {
  ...
  const mutation = useMutation({
    ...
    onError: (err: unknown) => setError(err instanceof Error ? err.message : "Error guardando egreso"),
  });
  ...
  return (
    <>
      <Header title="Nuevo egreso" subtitle="Registra un egreso del negocio" />
      <div className="flex-1 overflow-y-auto p-6">
        <form onSubmit={handleSubmit} className="card mx-auto max-w-2xl space-y-4 p-6">
          ...
          <div className="flex justify-end gap-3">
            <Button type="button" variant="secondary" onClick={() => navigate("/expenses")}>Cancelar</Button>
            <Button type="submit" isLoading={mutation.isPending}>Guardar egreso</Button>
          </div>
        </form>
      </div>
    </>
